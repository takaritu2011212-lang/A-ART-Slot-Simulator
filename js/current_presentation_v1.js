...
function installCurrentPresentation(){
  ...
  if(RendererV7.prototype.__currentPresentationPatched) return true;

  const proto=RendererV7.prototype;
  const originalSetText=proto.setText;
  if(typeof originalSetText!=='function') return false;

  proto.__currentPresentationPatched=true;
  proto.__currentPresentationOriginalSetText=originalSetText;

  const style=document.createElement('style');
  style.textContent=`
    #aartCurrentPresentation{
      margin:8px auto 0;
  ...
  function updateBox(renderer){
    ...
    const level=p.index ? ` ${p.index}段階` : '';
    const tier=p.tier==='premium' ? ' プレミア' : level;
    name.textContent=`${p.series || '通常'}${tier}`;
  }
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',installCurrentPresentation);
}else{
  installCurrentPresentation();
}
})();
