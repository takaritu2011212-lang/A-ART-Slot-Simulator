class RendererV6 {
    constructor(){
        ...
        this.catalog=this.buildCatalog();this.currentPresentation=null;this.pendingMismatch=false;this.pendingReason='';
    }
    buildCatalog(){
        const C=(text,roles)=>({tier:'common',text,roles});
        const S=(text,roles)=>({tier:'strong',text,roles});
        const P=(text,roles)=>({tier:'premium',text,roles});
        const common=[];
        const commonData=[
            ['青い光が一度だけ揺れた。',['ベル','リプレイ']],['同じ音が二度、重なった気がする。',['リプレイ','ベル']],['軽い金属音が残った。',['ベル']],['視線が中央へ流れた。',['ベル','リプレイ']],['一瞬だけ音が途切れた。',['ハズレ','リプレイ']],['もう一度、という感覚が残った。',['リプレイ']],['静かな繰り返しが始まる。',['リプレイ','ハズレ']],['同じ景色を見た気がする。',['リプレイ']],['緑が端に残った。',['スイカ','ベル']],['淡い緑の光が横切る。',['スイカ']],['何かが弾けるような気配。',['スイカ','チャンス目']],['緑と青、どちらかが反応した。',['スイカ','ベル']],['赤い点が一つだけ浮かんだ。',['弱チェリー','強チェリー']],['赤い光が端をかすめる。',['弱チェリー','強チェリー']],['小さな赤い違和感。',['弱チェリー']],['一瞬だけ赤、その後は静かだ。',['弱チェリー','ハズレ']],['チェリーらしい気配が残った。',['弱チェリー','強チェリー']],['赤か、ただの反射か。',['弱チェリー','ハズレ']],['図柄の間に小さなズレ。',['チャンス目','ハズレ']],['揃わないのに、形だけが気になる。',['チャンス目','ハズレ']],['一つだけ噛み合わない。',['チャンス目']],['視線が図柄の境目で止まった。',['チャンス目','リプレイ']],['何もないはずなのに引っ掛かる。',['チャンス目','ハズレ']],['音と図柄が少しだけずれた。',['チャンス目','リプレイ']],['小さなベル音が先に聞こえた。',['ベル']],['短い音が三つ続いた。',['ベル','リプレイ']],['軽い振動が一度だけ伝わった。',['ベル','ハズレ']],['音だけなら、いつも通りだ。',['ベル','ハズレ']],['静かな一回転。何かを待っている。',['ハズレ','リプレイ']],['時計の針が妙に気になった。',['ハズレ','リプレイ']],['店内の音に一つだけ違う音。',['ベル','ハズレ']],['右側だけが少し明るく見えた。',['スイカ','弱チェリー']],['左側に淡い色が残った。',['スイカ','弱チェリー']],['中央だけ、妙に静かだ。',['リプレイ','ハズレ']],['赤と緑、どちらかが潜んでいる。',['弱チェリー','スイカ']],['青と緑、どちらかが先に動いた。',['リプレイ','スイカ']],['青と赤、どちらかを見ろ。',['リプレイ','弱チェリー']],['ベルかリプレイ、その程度の違和感。',['ベル','リプレイ']],['ベルかスイカ、色だけが残った。',['ベル','スイカ']],['スイカかチェリー、片方の気配。',['スイカ','弱チェリー']],['赤い気配か、何もないか。',['弱チェリー','ハズレ']],['緑の気配か、何もないか。',['スイカ','ハズレ']],['青い気配か、何もないか。',['リプレイ','ハズレ']],['音の長さがいつもと少し違う。',['ベル','リプレイ']],['一度だけ画面が瞬いた。',['スイカ','弱チェリー']],['視界の端で赤が跳ねた。',['弱チェリー','強チェリー']],['視界の端で緑が跳ねた。',['スイカ','チャンス目']],['図柄を見る前に違和感が来た。',['チャンス目','リプレイ']],['何かを引いたような、引いていないような。',['チャンス目','ハズレ']],['いつもの音。だが一拍だけ長い。',['ベル','ハズレ']],['同じ音が戻ってきた。',['リプレイ']],['静かな再来。',['リプレイ','ハズレ']],['赤いものが一瞬だけ見えた。',['弱チェリー','強チェリー']],['緑のものが一瞬だけ見えた。',['スイカ']],['中央に小さな違和感。',['チャンス目','ベル']],['端から端へ、光が走った。',['スイカ','弱チェリー']],['何も揃わない音がした。',['ハズレ','チャンス目']],['次の一手を促すような音。',['ベル','リプレイ']],['気のせいなら、それで終わる。',['ハズレ','チャンス目']],['一瞬だけ、画面の端が明るい。',['ベル','スイカ']],['音のない瞬間が一つあった。',['ハズレ','チャンス目']]
        ];
        commonData.forEach(x=>common.push(C(x[0],x[1])));
        const strongData=[
        ...
        const strong=strongData.map(x=>S(x[0],x[1]));
    ...
    setText(s,kind='normal'){if(!this.e.text)return;this.e.text.className='演出-text';this.e.text.textContent=s;this.e.text.classList.add(kind);void this.e.text.offsetWidth;this.e.text.classList.add('flash');}
    isRareRole(){const n=game.currentRole?.name||'';return n.includes('強チェリー')||n.includes('チャンス目')||n.includes('スイカ')||n.includes('弱チェリー');}
    choosePresentation(){
        const roll=Math.random();
        let tier=roll<0.10?'premium':roll<0.40?'strong':'common';
        if(game.bonusPending&&Math.random()<0.18)tier='premium';
    ...
    evaluateLaw(){
        ...
        const roles=this.currentPresentation.roles||[];
        const matched=roles.includes(key);
        if(matched)return;
        // 法則ハズレは演出中には一切言わない。重要な局面だけ次ゲームへ持ち越す。
        const important=game.bonusPending||key==='強チェリー'||key==='チャンス目'||(this.isRareRole()&&Math.random()<0.18);
        if(important){this.pendingMismatch=true;this.pendingReason=key;}
    }
    startSpinEffect(){
        if(this.pendingMismatch){
            const after=['さっきの一回転、何か引っ掛からなかったか。','前の回転だけ、妙に印象に残っている。','今のところ、少しだけ違和感がある。','一つ前の回転が頭から離れない。','何かを見落とした気がする。'];
            this.setText(this.pick(after),'chance');this.pendingMismatch=false;this.currentPresentation=null;return;
        }
        this.currentPresentation=this.choosePresentation();
        const p=this.currentPresentation;
        this.setText(p.text,p.tier==='premium'?'premium':p.tier==='strong'?'strong':'normal');
    }
    startReels(){for(let i=0;i<3;i++){clearInterval(this.t[i]);const x=this.e['r'+(i+1)];this.t[i]=setInterval(()=>{const a=['7','BAR','ベル','リプ','スイカ','チェリー'];x.innerHTML=`<div class="reel-symbol">${this.pick(a)}</div><div class="reel-symbol">${this.pick(a)}</div><div class="reel-symbol">${this.pick(a)}</div>`;},65);x.classList.add('spinning');}}
    ...
    onReelStopped(i){
        // ボタン停止ごとの逐次台詞を基本にしない。重要演出だけ、ごく低確率で短い変化を入れる。
        if(!this.currentPresentation||this.currentPresentation.tier==='common')return;
        if(Math.random()>0.28)return;
        const lines=i===1?['一瞬だけ、空気が変わった。','まだ何も断定できない。']:i===2?['気配が続いている。','もう少しだけ見ておこう。']:['最後まで、何かが残った。','この回転は少し気になる。'];
        this.setText(this.pick(lines),this.currentPresentation.tier==='premium'?'premium':'strong');
    }
